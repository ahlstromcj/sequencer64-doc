...
\end{figure}
   ...
   click in the pattern editor, press "p" to select the pencil mode, then
   click/drag to add notes.  Press "x" to "eXit" from that mode.  Also remember
   ...
   Also new with \textsl{Sequencer64}, and not in \textsl{Seq64}, is the
   ...
   two.  Note that \textsl{Sequencer64} still needs a way to restart the
   progress bar without resetting it to the beginning of the pattern.
...
\end{figure}
   ...
   length of one measure.  For example, a sequence contain just one program
   ...
   And \textsl{Sequencer64} will, when it reads such a short sequence
...
\end{figure}
   ...
   Note that the Song Editor, which has no zoom functionality,
   has a resolution of 32 pulses per pixel, so, by default, it has
...
\label{subsec:seq64_pattern_editor_piano_roll}
   ...
   thin "event" bar just below it, and a taller "data" bar just below that.
   ...
   usable with some pretty crummy trackpads).
...
\end{figure}
   ...
   click in that section, then hit the "p" key to go into "paint" mode, and hit
   the "x" key to escape that mode.
...
%  the note to be selected.
   ...
   \index{keys!ctrl-v} and Paste (\texttt{Ctrl-V}) keys.
