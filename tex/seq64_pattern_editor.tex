...
%        now shows the application name (e.g. "seq64" versus the old
%        "Sequencer64" title.
...
%  \figureref{fig:pattern_editor_bottom_panel_items}.
   ...
   \textsl{Sequencer64} automatically scrolls
   ...
   \textsl{Sequencer64} also provides a way to restart the progress
...
% \end{figure}
   ...
   \textsl{Sequencer64} will, when it reads such a short pattern
   from a MIDI file,
%  (whether foreign or native to \textsl{Sequencer64}),
...
% \end{figure}
   ...
   \textsl{Sequencer64} includes a chord-generation option.
...
%  (through the "z", "Z", and "0" keystrokes only),
   ...
   there is no way to support that trick in \textsl{Sequencer64}.
...
\label{subsec:seq64_pattern_editor_piano_roll}
   ...
   necessity for editing (though \textsl{Sequencer64} is usable
...
\label{paragraph:seq64_pattern_editor_note_events}
   ...
   \textbf{Qt 5} version of \textsl{Sequencer64}.
   ...
   For now, \sectionref{paragraph:seq64_menu_file_options_mouse}, such as it
   is, will have to do.
...
\label{paragraph:seq64_pattern_editor_note_events_fruity}
   ...
   For now, see \sectionref{paragraph:seq64_menu_file_options_mouse}.
...
\end{figure}
   ...
   to make it match one's instrument.  See \sectionref{sec:seq64_usr_file}.
...
%  If \textsl{Sequencer64} is built with the \texttt{--enable-lfo} option, then
...
%  Note the controls in this window:
   ...
   \textsl{Sequencer64}, which then writes it to the MIDI output buss.

   \itempar{Record MIDI Data}{pattern editor!record midi data}
   This button routes incoming MIDI data into
   \textsl{Sequencer64}, which then saves the data to its buffer, and also
