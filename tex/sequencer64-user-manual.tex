...
%  the latest commit from the \textbf{master} branch.
   ...
   lhen proceed to \sectionref{subsec:introduction_lets_get_started}.
   ...
   JACK Master mode (via code from the \textsl{SooperLooper} 
   and \textsl{Seq32} projects), an additional \textsl{Qt 5} user-interface for
   Linux and for Window, a daemon mode, and play-list support with MIDI control.
   ...
   New features make song editing faster.
   ...
   With version 0.95.0, \textsl{Sequencer64} supports the
   ...
   0.96.0, but there's still a small amount of catching up to do.
...
\label{subsec:what_is_sequencer64}
   ...
   \cite{yoshimi} \cite{yoshimi2}, or the Windows built-in
   GS wave-table synthesizer.
   ...
   combines them.  They can be layed out as a song, and exported to a standard
   MIDI file.
...
\label{subsec:improvements}
   ...
   \begin{itemize}
      ...
      \item Tempo events are now supported, recordable, and editable.
      ...
      \item The user-interface can show more than one set at once (multiset), or
         more patterns in a single set (variset),
         for example, 8 rows by 8 columns,
      ...
      \item Support for reading \textsl{Cakewalk} "WRK" files.
         \index{Cakewalk}
         No support for writing them, however, and not all features of the
         "WRK" file are supported.
      \item Play-list support, with the option to control via MIDI events.
      ...
      \item Support for handling PPQN values other than 192, and for using
         the PPQN defined in the MIDI file, if desired.
...
%           personal preferences, and refactored into smaller modules.
         \item Much documentation was added to the code during reverse
            engineering.
         \item Debian packaging is incorporated into the project to make it
         ...
         \item The Qt 5 version (the future of \textsl{Sequencer64}) for both
            Linux and Windows can be built using qmake and the mingw compiler.
         \item The Windows version of \textsl{Sequencer64} provides either a
            portable zip package or an NSIS-based installer.
      \end{itemize}
...
\label{subsec:introduction_lets_get_started}
   ...
   First, make sure one has no other sound application running
   ...
   (On \textsl{Windows} just run it; the executable is \texttt{qpseq64.exe}).
...
\end{figure}
   ...
   Most of these features can be configured via the "rc" and "user" configuration
   ...
   Some of these items are not in the Qt interface.  Some might never be!
