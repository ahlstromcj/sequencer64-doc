...
\title{Sequencer64 User Manual 0.93.1}
\author{Chris Ahlstrom \\
...
\label{subsec:improvements}
   ...
   \begin{itemize}
      ...
      \item Tempo events are now supported, are recordable, and are editable.
         The first track can serve as a "tempo track".
...
\end{figure}
   ...
   label.  This is the \textbf{Tap Tempo} button (see
   \sectionref{subsec:seq64_patterns_panel_bottom} for more information).
   The light magenta button to the right of it, when click, logs the current
   tempo as a Set Tempo event.  To the right of that is a darker button that
   toggles the recording of all tempo changes.  (Currently, only those changes
   made with the main window's BPM spinner are saved; later, incoming tempo
   events will be recordable.)
   Another new item is the \textbf{Q} button,
