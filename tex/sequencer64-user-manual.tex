...
\title{Sequencer64 User Manual 0.9.11-rc}
\author{Chris Ahlstrom \\
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.75]{Sequencer64-0_9_10_1.png}
\end{figure}
...
\label{sec:introduction}
   ...
   \cite{sequencer64},
   through version 0.9.9.18, dated approximately through Feburary, 2016,
   and now through 0.9.11-rc, May 2016, using the latest commit from
   the \textbf{fixups} branch.
   .
   The lastest work has been getting the JACK support to work properly,
   in the \texttt{fixups} branch, now merged into \texttt{master}.
   Some additional work has been added to the \texttt{fixups} branch for
   version 0.9.9.18 and for 0.9.11 (pending),
   and that work is also documented here.
   ...
   callback with code adopted from the \textsl{SooperLooper} project.
   This code allows the \textsl{klick} metronome application to be driven by
   \textsl{Sequencer64}.
   So, overall, JACK and clocking are notably improved over \textsl{Seq24}.

   Version 0.9.11 adds a few features to make song editing slightly more
   convenient for the authoer, who uses ALSA for basic work.
   There is now a Pause button and a configuration Pause
   keystroke, which defaults to a period (".").
   There are still a few quirks to the Pause feature...
   are they bugs of features?
   Also, now a Shift-Left-Click on a pattern slot (pattern editor, main window)
   or on a pattern's name or M button (song editor) toggles the armed or
   mute status of all of the \textsl{other} active patterns.
   We also fixed a bug where the space-bar and the main window's Stop button
   interacted badly, and where the timing pills would not appear until the song
   started.  Some more optimizations have been done.
...
\label{subsec:what_is_sequencer64}
   ...
   \textsl{Sequencer64} is an continuation of
   \textsl{Sequencer24} (now obsolete), 
   ...
   one primary author of \textsl{Sequencer64},
...
\label{subsec:introduction_seq64_vs_others}
   ...
   while adding a few features we've found useful, to make it the "vi of
   sequencers".
...
\label{subsec:improvements}
   ...
   \textsl{Sequencer64}.  Note that we might have missed some minor new items.
   ...
   \begin{itemize}
      ...
      \item It is now possible to save the scale, key, and background sequence
         ...
         per-sequence.
      \item In the sequence/pattern editor, the window now scrolls to keep up
         with the progress bar for sequences that are longer than the width of
         the sequence editor window.  Still pending for the song editor.
      ...
      \begin{itemize}
         ...
         \item Shift-Left-Click on a pattern slot (in the pattern editor),
            pattern name or M (mute) (in the song editor)
            now toggles the status of all of the other active slots.
      ...
      \item On-going support for handling PPQN values other than the
         default value of 192.  Still not complete, but very use-able.
      ...
      \begin{itemize}
         ...
         \item Support for showing the currently-focussed (for editing in the
            ...
            not armed, and in cyan-on-black if armed.
         ...
         \item The last sequence brought up for editing is highlighted in the
            patterns panel.
