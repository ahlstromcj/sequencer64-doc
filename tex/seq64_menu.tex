...
\label{sec:seq64_menu}
   ...
   (\figureref{fig:seq64_main_screen}) is simple, but important to
   understand.
...
\label{subsec:seq64_menu_file}
   ...
   (Standard MIDI Format 0 or 1), save and load \textsl{Sequencer64} MIDI
   files.
...
%  \includegraphics[scale=0.75]{new/menu_file.png}
   \includegraphics[scale=0.85]{new/menu_file_0_96.png}
   \caption{Sequencer64 File Menu Items}
   \label{fig:seq64_menu_file_items}
\end{figure}

   There are some minor and unimportant differences between the \textsl{Qt} and
   the \textsl{Gtkmm} (shown in the figure) versions of this menu.
   We will comment on other differences only if they are not obvious to the
   user.
...
\label{subsec:menu_file_new}
   ...
   It also unloads the play-list, if it is loaded.
...
\end{figure}
   ...
   Note that \textsl{Sequencer64} can also open regular MIDI files and
   Cakewalk WRK files.
...
\label{subsubsec:seq64_menu_file_open}
   ...
   file.  Currently, the user must understand the format and use a text editor
   to edit the play-list file.
...
\end{figure}
   ...
   \textsl{Sequencer64}, one sees that all of the labelling, pattern
   ...
   and the output MIDI file after \textsl{Sequencer64} saves them; they
   are different.
   ...
   file.
   In its normal mode, \textsl{Sequencer64} saves this information by marking
...
\label{subsubsec:seq64_menu_file_import}
   ...
   or SMF 1 MIDI file as one or more patterns, one pattern per track in
   the MIDI file.
   Even long tracks, that aren't short loops, are read in properly.
   The difference from \textbf{Open} is that the destination set
   for the import can be specified, and the existing data in the
...
%  and had uploaded to the \textsl{GEnie} network service.
   ...
   Currently, only present in Gtkmm, but not Qt, which is a "bug" of sorts.
...
\label{subsubsec:seq64_menu_file_export}
   ...
   \begin{itemize}
      ...
      \item All of the triggers for a given track are consolidated.  Each
         ...
         one consolidated track that reconstructs the original playback
         layout.  This track is not needed (or even usable) in any
         non-Seq24-based MIDI sequencer; the events themselves are sufficient
         to play the performance exactly.  But the triggers for that are useful
         for further editing of the song/performance, as will be seen later,
...
\label{paragraph:seq64_menu_file_options_midi_clock}
   ...
   It is used to configure to what busses the MIDI clock and data gets dumped.
   ...
   \begin{itemize}
      \item What MIDI devices are connected to the computer.
         ...
         In \textsl{Windows}, the available devices are shown/
...
\end{figure}
   ...
   drive, in the manual/virtual mode.  One would have to use a JACK or ALSA MIDI
   ...
   output to that one buss, which is useful if only one buss is usable, and the
   MIDI song is, by default, routed to a non-existent buss.
   ...
   However, MIDI output can still be sent to those ports, and
...
%  produce a sound.
   ...
   The \textbf{Port Disabled} clock choice is a way to mark a port
   that one does not want to use, or to mark that the operating system
   (\textsl{Windows}, I am looking at \textsl{you})
   is locking/blocking/disabling the usage of that output port.
   Normally, this inaccessible port would cause \textsl{Sequencer64} to exit.
   With the port disabled, the inaccessible port is
   ignored, and the ports that are accessible can be used.
   When the \textsl{Windows} version of \textsl{Sequencer64}
   (\texttt{qpseq64} is first started, it may error out.
   It will then write "erroneous" configuration "rc" and "usr" files, which
   can be fixed to enable only available devices, which allows
   \texttt{qpseq64} to run.
%  See \sectionref{fig:qt5_prefs_clock_windows}, it shows this
%  new feature.

   \itempar{On (Pos)}{midi clock!on (pos)}
   Indicates that the MIDI clock will be sent to this buss.
   ...
   This item currently consists of one item, the Tempo Track number.
...
%  the MIDI port on the \textsl{E-MU XMidi1x1} USB cable,
   the four ports provided by \textsl{Timidity}.
%  , and the unlabelled
...
%  \textsl{Sequencer64} with this setup.
   ...
   We should rename this option to simply "manual" or "virtual"
...
\label{paragraph:seq64_menu_file_options_keyboard}
   ...
   These settings can also be modified by editing the appropriate configuration
   files, stored in one of the following directories, depending on the
   operating system:
   
   \begin{verbatim}
         /home/username/.config/sequencer64
         C:/Users/you/AppData/Local/sequencer64
   \end{verbatim}
...
%  \textbf{File / Options / Keyboard} page.
   ...
   This item shows the key labels in the lower-right corner of
   each loop/pattern slot in the Patterns window (the main window).
   This feature is useful for live playback and control of a song.
   This option is configurable in the "rc" configuration file.
   This option also enables the display of the pattern length, in
...
%  does exactly the same thing!
   ...
   any of the hot-keys, no matter how many, sets up the corresponding
   ...
   extended-keys section (\sectionref{subsubsec:seq64_patterns_pattern_slot}).
   ...
   Note that a mute-group key will be set only when
...
%  all patterns from other sets.
   ...
   Simultaneously, press one of the mute group keys: \textsl{Sequencer64}
   will save the currently-playing pattern slots into the corresponding mute
   group.
   \index{auto-shift}
   The default mute group keys need to be the shifted version of the key,
...
\label{paragraph:seq64_menu_file_options_ext_keys}
   ...
   completely filled.  It has its own section in the "rc" file.
...
% marked by "itempar".
   ...
   configuration file.
%  However, not all builds of \textsl{Sequencer64} will
...
%  But, if \textsl{Sequencer64} is built for
%  using the \textsl{Seq32} JACK support, then
   ...
   so that more hot-keys can be used without triggering menu function.
...
\end{figure}
   ...
   The \textbf{Sequencer64 Options} section
   currently contains a couple of new options.

   \index{keys!Mod4}
   \index{mouse!Mod4}
   \label{new_mod4_mode}
   \textbf{Mod4 key preserves add (paint) mode in song and pattern editors}.
   In order to work better with trackpads that don't permit simultaneous left
...
\label{paragraph:seq64_menu_file_options_jack_sync}
   ...
   This tab sets up options for JACK transport, if \textsl{Sequencer64}
