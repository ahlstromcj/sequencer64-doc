...
\label{subsec:menu_file_new}
   ...
   requiring a save) still needs some work.  We've made a lot of progress,
   though.}
...
\label{subsubsec:menu_file_open_save_as}
   ...
   It opens up the following standard GTK+2 file dialog, very similar to the 
...
\end{figure}
   ...
   \texttt{contrib/b4uacuse-seq24.midi}.  The latter is bigger.
...
% \begin{figure}[H]
%    \centering 
%    \includegraphics[scale=0.90]{new/imported_midi_song.png}
...
%  Some things to note.
%  First, the look of the main window can be modified by options for numbering
%  the slots (in the "rc" configuration file) and the style of drawing the
%  slots (in the "user" configuration file).
%  Second, the import can placed the imported data into any of the 32 available

   The import can placed the imported data into any of the 32 available
   sets.  Quite large songs can be built up by importing patterns from other
   MIDI files.
...
\begin{figure}[H]
   \centering 
   \includegraphics[scale=0.75]{menu/menu_file_export.png}
