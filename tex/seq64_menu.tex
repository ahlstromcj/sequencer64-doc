...
\end{figure}
   ...
   \textbf{New:}
   \index{new!grid numbers}
   If \textsl{Sequencer64} was build with the
   \texttt{SEQ64\_SEQNUMBER\_ON\_GRID} macro defined,
   as in \sectionref{subsubsec:seq64_build_macros},
   and the \textbf{Show key labels on sequence} option is turned on, the
   empty slots show the prospective sequence number.
...
\end{figure}
   ...
   If you don't like it, either turn off the option, or rebuild the code
   with the \texttt{SEQ64\_SEQNUMBER\_ON\_GRID} macro undefined.
