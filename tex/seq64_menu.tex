...
\end{figure}
   ...
   If unsaved changes are pending, the user should
...
\end{figure}
   ...
   \textbf{Warning:}
   \index{warnings!meta events}
   Also note that meta data not handled by \textsl{Sequencer64},
   such as Meta \texttt{FF 20 01 cc} (MIDI channel cc),
   and Meta \texttt{FF 21 01 pp} (MIDI port pp),
   are not saved
   (or even read in!)
   in the current release, and are lost.  The same is true upon import.
   But note that the two meta events mentioned above are also considered
   \textsl{obsolete}.
...
\label{subsubsec:seq64_menu_file_import}
   ...
   for the data in the destination set.
