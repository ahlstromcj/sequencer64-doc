...
\label{subsec:seq64_menu_file}
   ...
   \textsl{Sequencer64} should be able to handle any Format 0 or
   Format 1 standard files that any other sequencer exports.
...
\end{figure}
   ...
   This dialog let's one type a file-name, highlighting the first file (if any)
...
\label{subsubsec:seq64_menu_file_import}
   ...
   The \textbf{Import} menu entry allows one to import a Format 0
   or Format 1 MIDI file into one or more patterns, one pattern per track in
...
%  as shown in the following diagram, which shows an import of
   ...
   a transcription of an Eric Clapton tune made over 20 years ago.
%  and had uploaded to the \textsl{GEnie} network service.
   ...
   0 into sequences/patterns for each of the 16 MIDI channel.  It also puts all
   of the data in the 17th pattern (pattern 16), in case it is needed.
...
\label{subsubsec:seq64_menu_file_export}
   ...
   \begin{itemize}
      ...
      \item All of the triggers for a given track are consolidated.  Each
         ...
         one trigger that reconstructs the original playback
         layout.  This trigger is not needed (or even usable) in any other
...
\label{subsubsec:seq64_menu_file_options}
   ...
   It is not shown in some of the screenshot; we are too lazy to re-do those
...
\end{figure}
   ...
   the playing of MIDI in this manner.  Also, the "user" configuration file
   can change the names of the ports, causing further confusion.
...
\end{figure}
   ...
   \itempar{Extended keys}{keyboard!extended keys}
   This block of fields in the \textbf{Options / Ext Keys} tab
   provides shortcut keys for more operations of \textsl{Sequencer64}.
...
\end{figure}
   ...
   which shows the current state of this flag.  Note that this flag is also
   stored in the "rc" configuration file, as well as this hotkey value, which
