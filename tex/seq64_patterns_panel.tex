...
\end{figure}
   ...
   keys.  For example, we have the Fluxbox window manager
...
\end{figure}
   ...
   Pressing the "keep queue" hot-key (we like the backslash key)
   \index{rc file}
   assigned in the "rc" file
   activates permanent queue mode until one click the regular
   (i.e. temporary) queue function hot-key,
   ...
   Also note the "queued-replace" functionality, described a bit later.
   ...
   Again, this is a very inconvenient default, so an alternative key mapping
...
\end{figure}
   ...
   This figure is way out of date, and does not show the new items, marked as
   "(new)", in the list below.
   ...
   \begin{enumber}
      ...
      \item \textbf{Log Current Tempo} (new)
      \item \textbf{Record Tempo Changes} (new)
      \item \textbf{Keep-Queue Status} (new)
      \item \textbf{Name}
      \item \textbf{Set}
...
\end{figure}
   ...
   \itempar{Log Current Tempo}{pattern!log tempo}
   ...
   \itempar{Record Tempo Changes}{pattern!record tempo}
