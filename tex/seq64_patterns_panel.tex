...
\end{figure}
   ...
   \itempar{ALSA/JACK Mode}{pattern!ALSA/JACK mode}
...
%  lifting \texttt{Alt}.  Not yet sure exactly what this means.
   ...
   window to disable the menu, which makes those operations available.
   ...
   at the end of a pattern.  Also note the "keep queue" functionality described
   below.
...
\end{figure}
   ...
   activates permanent queue mode until one presses the regular
   (i.e. temporary)
   queue function. 
   After pressing the "keep queue" key, individual pattern
   toggles occur at the end of the current pattern rather than immediately.
   “Keep queue” mode is canceled by pressing the normal Queue key.
   ...
   by default (which cannot work, because \textsl{Sequencer64} now uses
   \texttt{Ctrl-L} as a way to click the "L" button).
   Replacement is a form of muting/unmuting.  When the "replace"
   key is pressed while clicking a sequence, that sequence is unmuted, and all
   of the other sequences are muted.  Again, this is a very inconvenient
   default, so an alternative key mapping is provided.
   Note that the "replace" functionality is a form of "solo".
   Also note that the "replace" functionality is implemented via MIDI
   control, where the MIDI control can be activated, but then the user has to
   click on the desired sequence.  

   \index{queue!replace}
   \index{queue!solo}
   \textsl{Sequencer64} provides an extension to the replace/solo functionality
   called "queued-replace" or "queued-solo".  In this feature, when
   ...
   \begin{enumber}
      ...
      \item To end the "queued-replace" mode, click the normal "queue hot-key.
   \end{enumber}
