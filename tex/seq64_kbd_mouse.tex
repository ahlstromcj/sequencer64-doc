...
%  Any volunteers to fill in the table?
   ...
   \index{keys!focus}
   Also note that the effect that keystrokes have will generally depend upon
   which window has the keyboard and mouse focus.

   \index{keys!qt}
   And it must be noted that the Qt 5 user-interface does not (yet) support the
...
\label{subsec:kbd_mouse_pattern_editor}
   ...
   Here are the keystrokes handles by the main frame of the piano roll:
   ...
   \begin{itemize}
      ...
      \item \texttt{Delete}.  Deletes (not cuts!) the currently-selected notes
         in the piano roll.
         This operation can be undone withe the \textbf{Undo} button.
   \end{itemize}
...
\label{subsubsec:kbd_mouse_pattern_editor_piano_roll}
   ...
   \begin{itemize}
      \item \textsl{p}:  Enters the paint mode, until right-click is pressed or
         until the \texttt{x} key is pressed.  In this mode, notes can be added
